...
public struct UInt128: Equatable, Hashable {
    ...
    internal func divideBy1Billion() -> (quotient: UInt128, remainder: Int) {
        ...
        guard !(quotient.high == 0 && quotient.low == 0) else {
        ...
        for i in 0...3 {
            ...
            switch i {
            case 0: quotientI = quotient.high.upper32bits
            case 1: quotientI = quotient.high.lower32bits
            case 2: quotientI = quotient.low.upper32bits
            case 3: quotientI = quotient.low.lower32bits
            default: quotientI = 0
            }
            remainder += quotientI
            // quotient[i] = Int(remainder / DIVISOR)
            switch i {
            case 0: quotient.high = (((remainder / denominator) << 32) | quotient.high.lower32bits)
            case 1: quotient.high = ((remainder / denominator).lower32bits | quotient.high.getBitsUnshifted(0...31))
            case 2: quotient.low = (((remainder / denominator) << 32) | quotient.low.lower32bits)
            case 3: quotient.low = ((remainder / denominator).lower32bits | quotient.low.getBitsUnshifted(0...31))
            default: break
            }
            // Store the remainder
...
public struct BSONDecimal128: Equatable, Hashable, CustomStringConvertible {
    ...
    private var isNaN: Bool { self.value.high.getBits(1...5) == Self.combinationNaN }
    /// Determines if the value is Infinity  by checking if bits 1-5 are equal to 1 and bit 6 is 0 ignoring sign bit
    private var isInfinity: Bool { self.value.high.getBits(1...5) == Self.combinationInfinity }
    /// Determines if the value is Negative
    private var isNegative: Bool { self.value.high.getBit(0) == 1 }
    ...
    internal init(high: UInt64, low: UInt64) {
        self.value = UInt128(high: high, low: low)
    }
    ...
    public init(_ data: String) throws {
        ...
        var significand = UInt128(high: product.high, low: product.low)

        let (result, didOverflow) = significand.low.addingReportingOverflow(significandLoDigits)
        significand.low = result
        ...
        value.high |= biasedExponent << (63 - Self.exponentLength)
        value.high |= significand.high

        value.low = significand.low
    ...
    private func toString() -> String {
        ...
        if self.value.high.getBits(1...2) == 0b11 {
            if self.isInfinity {
            ...
            exponent = Int(self.value.high.getBits(3...(Self.exponentLength + 2)))
            significandPrefix = (self.value.high.getBit(20) + 0b1000)
        } else {
            // Decimal interchange floating-point formats c,2,i
            exponent = Int(self.value.high.getBits(1...Self.exponentLength))
            significandPrefix =
                self.value.high.getBits(
        ...
        significand128.high = UInt64(
            significandPrefix.getLeastSignificantBits(4) << (Self.trailingSignificandLength - 64))
            | self.value.high.getLeastSignificantBits(Self.trailingSignificandLength - 64)
        significand128.low = self.value.low
        ...
        if significand128.high == 0 && significand128.low == 0 {
            isZero = true
        } else if significand128.high.upper32bits >= 0x20000 {
            /*
...
extension BSON.Decimal128 {
    var toString: String {
        let mirror = Mirror(reflecting: self)
        guard let low = mirror.children.first(where: { $0.label == "low" })?.value as? UInt64,
              let high = mirror.children.first(where: { $0.label == "high" })?.value as? UInt64 else {
            return "NaN"
        }
        
        let stringValue = BSONDecimal128(high: high.byteSwapped, low: low.byteSwapped)
